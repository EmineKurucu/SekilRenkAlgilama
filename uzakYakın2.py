...
while True:
    ...
    if target_color and target_shape:  # Only search for the target color and shape
        for color_name, (mask, color) in color_masks.items():
            if color_name == target_color:  # Search only for the target color
                contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                for cnt in contours:
                    shape = detect_shape(cnt)
                    if shape == target_shape:  # Match the target shape
                        x, y, w, h = cv2.boundingRect(cnt)
                        cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
                        cv2.putText(frame, f"Hedef Bulundu: ", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                        cv2.putText(frame, "Hedef Belirlendi: " + {target_color} + {target_shape}, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                        break  # Stop once the target is found
    else:
        # Search for the target if not found yet
        for color_name, (mask, color) in color_masks.items():
            contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            for cnt in contours:
                shape = detect_shape(cnt)
